...
class PowerLaw(Component):
    ...
    def getgradient(self,parameter):
        ...
        if parameter==p1:
            ...
            self.gradient[1]=mask*self.powerlaw(A,r)*np.log(self.Estart/E)
            # self.gradient=mask*(A*(E/self.Estart)**(-r))*(-1)*np.log(E/self.Estart) #the two equations are equivalent
    ...
    def autofit_(self,spectrum,istart,istop):
        ...
        #added to remove pixel values smaller than 1 since they give a problem with the log
    ...
    def autofit_matrix(self, spectrum, istart, istop):
        """
        Weighted linear least squares where the variance is given by the square root of the
        signal
        ...
        A[:, 1] = -np.log(self.energy_axis[istart:istop][boolean]/self.energy_axis[0])
    ...
    def autofit_twowindow(self, spectrum, istart, istop):
        ...
        I0 = np.sum(y[:len(y) // 2])
        I1 = np.sum(y[len(y) // 2:])
        r = 2. * np.log(I0 / I1) / np.log(x1 / x0)  # the usual power-law exponent
    ...
    def autofit_areas(self, spectrum,istart,istop):
        ...
        #in this case it is found experimentally instead of proven (so can be wrong)
        A = (k * I2 / ((E[istop]/E[0]) ** k - (E[imiddle]/E[0]) ** k))/E[0]
        # sanity check
        if (r < 1.0) or (r > 10):
            r = 3.0
            A = spectrum.data[0]
