...
class AlignCrossCorrelation(Align):
    ...
    signal_range: tuple
        Indicates the region on which the cross correlation should be performed.
        (default: None)
    cropping: bool
    ...
    def reference(self, ref):
        ...
        if self.subpixel:
            self._reference = self.interpolate_int(ref[ind0:ind1]-ref[ind0:ind1].mean(),
                                                   self.interp)
        else:
    ...
    def interp(self, index):
        if index<1 or not isinstance(index, int):
            raise ValueError(r'The given interpolation step should be an integer larger than 0')
        self._interp = index
    ...
    def interpolate_int(self, data, index):
        """
        Function which interpolates the data with the number of index points. So if index is two, the
        interpolated data will contain twice as many points.
    ...
    def determine_shift(self):
        ...
        for index in tqdm(np.ndindex(shape)):
            islice = np.s_[index]
            signal = self.multispectrum.multidata[islice][ind0:ind1] - \
                     self.multispectrum.multidata[islice][ind0:ind1].mean()
            if self.subpixel:
        ...
        # self.index_shift = self.index_shift - self.index_shift[0] #this assumes that the first specturm is the reference
        if self.subpixel:
            self.shift = -1 * self.index_shift * self.multispectrum.dispersion/self.interp
        else:
