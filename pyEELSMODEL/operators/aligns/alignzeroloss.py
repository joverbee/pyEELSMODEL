...
class AlignZeroLoss(Align):
    ...
    other_spectra: List of Multispectra
        The other multispectra which can be aligned with the same parameters.
        (default: None)
    signal_range: tuple
        Indicates the region on which the zero loss fitting should be performed.
        (default: None)
    model_type: string
        Selecting which model is used to fit the zero loss peak (default: 'Gaussian')
    cropping: bool
        Indicates if the edges of the signal are cropped out when aligning the zero loss peak.
        (default: False)
    use_bounds: bool
        Use the boundaries for the fitting. This could make the fit more stable.
        (default: True)

    """
    def __init__(self, multispectrum, other_spectra=None, signal_range=None, model_type='Gaussian',
                 cropping=False, use_bounds=True):
    ...
    def estimate_start_parameters(self):
        """
        Estimates the starting values for every scan position to fit the zero loss peak.
        The A value is found by taking the maximum.
        The centre is chosen as the coordinate at which the maximum occurs
        Sigma is taken as a fixed value of 1. (better guess can be tried but it seems to be stable)
        It stores the result is the start_parameters attribute.
        """
        # The lorentzian and gaussian model both have three parameters.
        start_param = np.zeros((self.multispectrum.xsize, self.multispectrum.ysize, 3))
        ...
        shift = (np.argmax(self.multispectrum.multidata[:,:,ind0:ind1], axis=(2))+ind0)
        start_param[:, :, 1] = self.multispectrum.energy_axis[shift]

        # fwhm is guess from the width of the summed spectrum.
        # this says that the width does not change over the scan (which is normally true)
        # zl = self.fast_aligned.multidata.sum((0,1))
        # coor = np.argwhere(zl>0.5*np.max(zl))

        # start_param[:, :, 2] = self.multispectrum.energy_axis[coor[-1]]-self.multispectrum.energy_axis[coor[0]]
        start_param[:, :, 2] = 1
    ...
    def make_zeroloss_model(self):
        ...
        comp.parameters[1].setboundaries(self.signal_range[0], self.signal_range[1], force=True)

        # if self.signal_range is not None:
        #     comp.parameters[1].setboundaries(self.signal_range[0], self.signal_range[1], force=True)
        #     print('changed_boundaries 1')
        # else:
        #     comp.parameters[1].setboundaries(self.multispectrum.energy_axis[0], self.multispectrum.energy_axis[-1])
        #     print('changed_boundaries 1')
    ...
    def calculate_model(self):
        ...
        #if the start parameters are not yet calculated do it before the model fitting
