...
class CoreLossSimulator(Operator):
    ...
    by convolving a powerlaw with the background to include multiple scattering.
    ...
    def __init__(self, multispecshape, elements, edges, maps, tlambda_map,
        ...
        self.Ep_map = 22*np.ones_like(self.tlambdas) #energy of bulk plasmon at each probe position
        self.Ew_map = 5*np.ones_like(self.tlambdas) #width of the bulk plasmon at each probe position
        self.n_plasmon = 5 #number of plasmon included
        self.fwhm = 1 #eV
        self.llspectrumshape = Spectrumshape(multispecshape.dispersion, -self.fwhm*10, multispecshape.Esize)
    ...
    def make_lowloss(self):
        ...
        for index in tqdm(np.ndindex(self.scan_size)):
            islice = np.s_[index]
            A  = zlp.data.sum()
            plasmon = Plasmon(self.llspectrumshape, A=A, Ep=self.Ep_map[islice],
                              Ew=self.Ew_map[islice], n=self.n_plasmon,
    ...
    def _calculate_convoluted_background(self, llcomp):
        ...
        nspecshape = Spectrumshape(ndispersion, 100, 2 * self.spectrumshape.size)
    ...
    def make_elements(self):
        ...
        for elem, edge, onset in zip(self.elements, self.edges, self.onsets):
            comp = ZezhongCoreLossEdgeCombined(self.spectrumshape, 1, self.E0, self.alpha,
                                               self.beta, elem, edge,
                                               eshift=onset,
                                               q_steps=self.qsteps)
    ...
    def make_coreloss(self):
        ...
        for index in tqdm(np.ndindex(self.scan_size)):
            ...
            # bg = self._calculate_convoluted_background(self.ll[islice])
    ...
    def add_shift(self):
        ...
        self.shift2d = np.reshape(shift1d, (self.ll.xsize, self.ll.ysize)).astype('int')
        ...
        for index in tqdm(np.ndindex(self.scan_size)):
            islice = np.s_[index]
            cldata[islice] = np.roll(self.multispectrum.multidata[islice], self.shift2d[islice])
            lldata[islice] = np.roll(self.ll.multidata[islice], self.shift2d[islice])

        cldata = cldata[:,:,self.shift2d.max():-self.shift2d.max()]
        lldata = lldata[:,:,self.shift2d.max():-self.shift2d.max()]

        cl_offset = self.multispectrum.offset + self.shift2d.min()*self.multispectrum.dispersion
        cl_energy = np.arange(cldata.shape[2])*self.multispectrum.dispersion + cl_offset
