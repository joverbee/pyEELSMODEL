...
class BackgroundRemoval(Operator):
    ...
    def signal_range(self, signal_range):
        """
        Set the signal range and first validates if the given signal_range is valid.
        The signal range can have two regions such that a interpolation on the
        background can be performed.
        ...
        if not self.two_area:
            if signal_range[0]<self.spectrum.energy_axis[0] or signal_range[1]>self.spectrum.energy_axis[-1]:
                raise ValueError(r'Invalid signal range')
            if signal_range[0]>=signal_range[1]:
                raise ValueError(r'First number of signal range must be smaller than the second value')

        else:
            for tup in signal_range:
                if tup[0]<self.spectrum.energy_axis[0] or tup[1]>self.spectrum.energy_axis[-1]:
                    raise ValueError(r'Invalid signal range')
                if tup[0] >= tup[1]:
                    raise ValueError(r'First number of signal range must be smaller than the second value')

            if signal_range[0][1] > signal_range[1][0]:
                raise ValueError(r'The end energy of the first energy range is larger than the beginning of the second')
    ...
    def do_linear_fitting(self, bool):
        if bool and (self.model_type!='FastBG' and self.model_type!='Polynomial'):
            raise ValueError(r'This model type cannot perform a linear fit over the '
                             r'background. Set linear fitting to false if you want to '
                             r'use this type of background model')
    ...
    def make_background_model(self):
        ...
        if self.model_type == 'Powerlaw':
            ...
            comp = FastBG2(specshape, A1=1, r1=self._r_values[0],A2=1, r2=self._r_values[1])
    ...
    def determine_fast_fit_parameters(self):
        ...
        Fast fitting only works for the powerlaw and exponential background models

        """
        # if type(self.spectrum) is not Spectrum:
        #     raise ValueError(r'Multispectrum used is no Spectrum')
    ...
    def determine_multi_fast_fit_parameters(self):
        ...
        # self.spectrum.show_excluded_region()

        if self.model_type == 'Powerlaw' or self.model_type == 'Exponential':
            fast_matrix = np.empty((self.spectrum.xsize, self.spectrum.ysize, 2))
            for index in tqdm(np.ndindex(fast_matrix.shape[:-1])):
                ...
                fast_matrix[islice + (0,)] = self.model.getfreeparameters()[0].getvalue()
                fast_matrix[islice + (1,)] = self.model.getfreeparameters()[1].getvalue()

            self.multi_fast_fit = fast_matrix

        else:
            print('This model type does not support a fast fit method. Only works for Powerlaw or Exponential')
    ...
    def include_areas(self):
        ...
        if self.two_area:
            ...
            self.spectrum.set_include_region(self.indices[1][0], self.indices[1][1])
        else:
    ...
    def set_fit_type(self):
        ...
        if self.do_linear_fitting:
            ...
            if self.non_linear_fitter == 'ML':
                ...
                fit = LSQFitter(self.spectrum, self.model, use_bounds=True, method='trf')
        self.fitter = fit
    ...
    def calculate_multi(self):
        ...
        spectrum will be set the same as it was before this function was called.
    ...
    def fast_calculate_multi(self):
        ...
        if self.multi_fast_fit is None:
            logger.info('Determination of fast fit paramters is peformed first')
            self.determine_multi_fast_fit_parameters()

        fast_back = self.spectrum.copy() #background subtracted
        rem_sig = self.spectrum.copy() #calculated background
        ndata = np.empty((self.spectrum.xsize, self.spectrum.ysize, self.spectrum.size))
        rem_dat = np.empty(ndata.shape)
        for index in tqdm(np.ndindex(fast_back.multidata.shape[:-1])):
            islice = np.s_[index]
            self.model.getfreeparameters()[0].setvalue(self.multi_fast_fit[islice+(0,)])
            self.model.getfreeparameters()[1].setvalue(self.multi_fast_fit[islice+(1,)])
    ...
    def show_fit_result(self, index=(0,0), integration_range=None, use_mean=False):
        """
        ...
        ax.fill_between(self.spectrum.energy_axis, 0, 1.1*self.spectrum.data.max(),
                        where=np.invert(self.spectrum.exclude), color='green', alpha=0.5,
        ...
        if integration_range is not None:
            ...
            ax.fill_between(self.spectrum.energy_axis, self.model.data, self.spectrum.data,
                            where=boolean, color='blue', alpha=0.5,
    ...
    def quantify_from_edge(self, integration_range, element, edge, E0, alpha, beta,
                           ll=None):
        """
