...
XU = (0.52,0.42,0.30,0.29,0.22,0.30,0.22,0.16,0.12,0.13,0.13,0.14,0.16,0.18,0.19,0.22,0.14,0.11,0.12,0.12,0.12,0.1,0.1,0.1)
IE3 = (73.0,99.0,135.0,164.0,200.0,245.0,294.0,347.0,402.0,455.0,513.0,575.0,641.0,710.0,779.0,855.0,931.0,1021.0,1115.0,1217.0,1323.0,1436.0,1550.0,1675.0)
IE1 = (118.0,149.0,189.0,229.0,270.0,320.0,377.0,438.0,500.0,564.0,628.0,695.0,769.0,846.0,926.0,1008.0,1096.0,1194.0,1142.0,1248.0,1359.0,1476.0,1596.0,1727.0)
EK = (13, 25, 55, 111, 188, 284, 402, 532, 685, 867, 1072, 1305, 1560, 1839, 2146, 2472, 2822, 3203, 3607, 4038, 4493, 4966)
...
def gdos_k(E, qa0sq, Z):
    ...
    if kHsq>=0.0:
        ...
        noemer = Zs**4*pc.R()**2*((Q-kHsq+1)**2+(4*kHsq))**3*(1-np.exp(-2*np.pi/akH))+eps
        df_dE = teller/noemer
    else:
        y = -(1/akH)*np.log((Q+1-kHsq+2*akH)/(Q+1-kHsq-2*akH+eps))
        teller = 256*E*(Q+kHsq/3+1/3)*np.exp(y)
        noemer = Zs**4*pc.R()**2*((Q-kHsq+1)**2+(4*kHsq))**3+eps
...
def gdos_l(E, qa0sq, Z):
    ...
    if kHsq >= 0.0:
        ...
        y = (-1/akH)*np.log((Q+0.25-kHsq+akH)/(Q+0.25-kHsq-akH))
        C = np.exp(y)
        D = 1.0
    if E<=EL1:
        G = 2.25*Q**4-(0.75+3.0*kHsq)*Q**3+(0.59375-0.75*kHsq-0.5*kHsq**2)*Q**2+(0.11146+0.85417*kHsq+1.8833*kHsq**2+kHsq**3)*Q+0.0035807+kHsq/21.333+kHsq**2/4.5714+kHsq**3/2.4+kHsq**4/4
        A = ((Q-kHsq+0.25)**2+kHsq)**5
    else:
        G = Q**3-((5/3)*kHsq+11./12.)*Q**2+(kHsq**2/3+1.5*kHsq+65./48.)*Q+kHsq**3/3+0.75*kHsq**2+kHsq*23./48.+5./64.
...
def convergence_correction_factor(alpha, beta, theta_sampling):
    ...
    y = (beta ** 2 + theta_sampling[mask_2] ** 2 - alpha ** 2) / (2 * beta * theta_sampling[mask_2])

    F[mask_1] = min(alpha, beta) ** 2 / alpha ** 2

    term1 = np.arccos(x)
    term2 = (beta ** 2 / alpha ** 2) * np.arccos(y)
    wortel = np.sqrt(4 * alpha ** 2 * beta ** 2 - ((alpha ** 2 + beta ** 2 - theta_sampling[mask_2] ** 2) ** 2))
...
def dsigma_dE_hydrogenic(energy_axis, Z, ek, E0, beta, alpha, shell='K', q_steps=100):
    """
    Calculates the differential cross section for a given energy axis. It uses the approximation of a convergent
    beam (alpha).
    ...
    for i in range(energy_axis.size):
        ...
        if E>ek:
            ...
            for j in range(logqa0sq_axis.size):
                ...
                theta = 2.*np.sqrt(np.abs(R*(qa0sq-qa0sq_min)/(4.*gamma**2*T)))
                # integral += gdos(E, qa0sq, Z)*lnqa0sqstep
                integral += correction_factor_kohl(alpha, beta, theta)*gdos(E, qa0sq, Z)*lnqa0sqstep # correction does not work
