...
def test_fast_align_1():
    ...
    for i in range(len(shf_l)):
        ...
        assert align.aligned_others[0].energy_axis[np.argmax(align.aligned_others[0].sum().data)] == 50
        assert align.aligned_others[1].energy_axis[np.argmax(align.aligned_others[1].sum().data)] == 100
...
def test_cross_correlation_signal_range():
    ...
    for i in range(len(shf_l)):
        s, s1, s2, real_shift = make_artificial_dataset(shf_l[i][0], shf_l[i][1], return_shift=True)
        align = AlignCrossCorrelation(s, other_spectra=[s1, s2], signal_range=(-50,50))
        align.perform_alignment()
