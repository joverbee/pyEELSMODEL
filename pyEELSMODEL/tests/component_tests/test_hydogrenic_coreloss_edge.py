...
def test_set_edge():
    ...
    OK= HydrogenicCoreLossEdge(specshape, A=1e3, E0=300e3, alpha=5e-3, beta=20e-3, element= 'O', edge='K')
    assert OK.edge == 'K'

    TiL = HydrogenicCoreLossEdge(specshape, A=1e3, E0=300e3, alpha=5e-3, beta=20e-3, element= 'Ti', edge='L')
    assert TiL.edge == 'L'

    with pytest.raises(ValueError):
        HydrogenicCoreLossEdge(specshape, A=1e3, E0=300e3, alpha=5e-3, beta=20e-3, element='Ti', edge='M')
    with pytest.raises(ValueError):
        HydrogenicCoreLossEdge(specshape, A=1e3, E0=300e3, alpha=5e-3, beta=20e-3, element='C', edge='L')
    with pytest.raises(ValueError):
        HydrogenicCoreLossEdge(specshape, A=1e3, E0=300e3, alpha=5e-3, beta=20e-3, element='Ag', edge='L')
...
def test_set_onset_energy():
    ...
    CaL= HydrogenicCoreLossEdge(specshape, A=1e3, E0=300e3, alpha=5e-3, beta=20e-3, element= 'Ca', edge='L')
