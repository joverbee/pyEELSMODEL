...
def test_check_same_settings():
    ...
    assert s0._check_not_same_settings(s1) ==  False
    assert s0._check_not_same_settings(s2) ==  True
    assert s0._check_not_same_settings(s3) ==  True
    assert s0._check_not_same_settings(s4) ==  True
...
def test_set_exclude_region():
    ...
    assert s0.exclude[10:110].all() == True
    assert s0.exclude[:10].all() == False
    assert s0.exclude[110:].all() == False
...
def test_reset_exclude_region():
    ...
    assert s0.exclude[10:90].all() == False
    assert s0.exclude[90:110].all() == True
...
def test_inter_to_other_energy_axis():
    ...
    dif = s.energy_axis[np.argmax(s.data)] - sint.energy_axis[np.argmax(sint.data)]
...
def test_rescale_spectrum():
    ...
    dif = s.energy_axis[np.argmax(s.data)] - s0.energy_axis[np.argmax(s0.data)] - 50
    assert np.abs(dif) < 0.1

    s1 = s.rescale_spectrum(0.5,0)
    dif = s.energy_axis[np.argmax(s.data)] - s1.energy_axis[np.argmax(s1.data)]-100
    assert np.abs(dif) < 0.1
